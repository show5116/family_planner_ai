...
*(참고: NestJS 측 환경변수에 이 값을 등록해두고 Axios/Fetch 요청 시 공통 헤더로 주입해 사용하시기 바랍니다.)*
...
| `room_id` | `string` | No | `"default_room"` | 채팅방을 식별하는 고유 ID. `user_id`와 결합되어 해당 스레드의 대화 이력이 복원됩니다. |
...
*💡 대화 이력 작동 원리: `user_id`와 `room_id`를 조합해 스레드(Thread)를 식별합니다. 사용자가 동일한 `room_id`로 메시지를 보내면, AI는 이전 대화 문맥(Context)을 모두 기억하고 답변합니다.*
...
export class AiFamilyPlannerService {
  ...
  async sendMessageToAi(userId: string, roomId: string, message: string, agent?: string) {
    try {

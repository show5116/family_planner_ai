...
*💡 대화 이력 작동 원리: `user_id`와 `room_id`를 조합해 스레드(Thread)를 식별합니다. 새 대화를 시작할 때는 `room_id` 없이 보내면 서버가 방금 만든 `room_id`를 응답으로 줍니다. 이어지는 대화에서는 받은 `room_id`를 계속 포함해 전송해야 AI가 문맥을 기억합니다.*
